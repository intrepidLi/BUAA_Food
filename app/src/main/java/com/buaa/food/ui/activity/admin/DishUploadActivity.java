...
public final class DishUploadActivity extends AppActivity
    ...
    public static void start(BaseActivity activity, String phone, String password, OnRegisterListener listener) {
        Intent intent = new Intent(activity, DishUploadActivity.class);
...
//
            ...
            if (dataBaseHelper.checkPhone(phone)) {
                Toast.makeText(DishUploadActivity.this, "User Already Exists", Toast.LENGTH_SHORT).show();
                mCommitView.showError(3000);
            }

            if (dataBaseHelper.insertInitUser(username, password, phone)) {
                Toast.makeText(DishUploadActivity.this, "SignUp Successfully", Toast.LENGTH_SHORT).show();
                mCommitView.showSucceed();
                UserAuth.setLocalUserPhone(phone);
                HomeActivity.start(getContext(), MineFragment.class);
            } else {
                Toast.makeText(DishUploadActivity.this, "SignUp Failed", Toast.LENGTH_SHORT).show();
                mCommitView.showError(3000);
            }
        }
